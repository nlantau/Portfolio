...
root = r"/Users/nlantau/Docs/GitHub/DT256CHT20/Seminar_2/"
bin_path = root + r"Cpp/"
...
def run_C_program(bin_name: str) -> list:
    ...
    res = []
    try:
        if os.path.exists(c_file := os.path.abspath(root + r"Cpp/./" + bin_name)):
            for i, path in enumerate(N):
                progress(i, len(N) - 1, status=path)
...
def main(bin_name: str, res_file_name: str, iterations: int):
    ...
    print(f"\n{' Test Done ':-^56}\n")
    info = (
        f"> Results stored in: {Fore.GREEN}{res_file + res_file_name}{Fore.RESET}\n\n"
    )
    for ch in info:
        print(f"{ch}", end="", flush=True)
        time.sleep(0.01)
